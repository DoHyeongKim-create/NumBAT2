...
C
      ...
      if (debug .eq. 1) then
        write(*,*)
        write(*,*) "EM_mode_energy_int_Ez: Starting"
      endif
C
      if ( nnodes .ne. 6 ) then
        write(ui,*) "EM_mode_energy_int_Ez: problem nnodes = ", nnodes
        write(ui,*) "EM_mode_energy_int_Ez: ",
     *              "nnodes should be equal to 14 !"
        write(ui,*) "EM_mode_energy_int_Ez: Aborting..."
        stop
      endif
c
      call quad_triangle (nquad, nquad_max, wq, xq, yq)
      if (debug .eq. 1) then
        write(ui,*) "EM_mode_energy_int_Ez: nquad, nquad_max = ",
     *              nquad, nquad_max
      endif
C
      do i=1,nval
...
c           sol_el_1 : H-field
            sol_el_1(ind_jp) = - z_tmp1 * ii
cc            sol_el_1(ind_jp) = z_tmp1 * beta1
          enddo
          do i=nnodes+1,13
...
c       Scalar product
     ...
     *          + z_tmp1
      ...
      if (debug .eq. 1) then
        write(*,*) "EM_mode_energy_int_Ez: Ending"
      endif
C
